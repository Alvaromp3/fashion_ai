...
if [ -f "$DEFAULT_REPO_VIT" ]; then
  export ML_VIT_PATH="$DEFAULT_REPO_VIT"
elif [ -f "$LEGACY_USER_VIT" ]; then
  export ML_VIT_PATH="$LEGACY_USER_VIT"
else
  echo -e "${YELLOW}ML: ERROR — no se encontró ViT en:${NC}"
  echo -e "  $DEFAULT_REPO_VIT"
  echo -e "  ni $LEGACY_USER_VIT"
  echo -e "${YELLOW}Coloca best_model_17_marzo.keras en ml-service/models/ o exporta ML_VIT_PATH${NC}"
  exit 1
fi
