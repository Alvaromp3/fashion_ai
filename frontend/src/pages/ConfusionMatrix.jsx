...
const ConfusionMatrix = ({ embedded = false }) => {
  ...
  const [vitRealImageUrl, setVitRealImageUrl] = useState(null)
  ...
  useEffect(() => {
    ...
    fetchCnnData()
    fetchVitData()
    // Para ViT entrenado en fotos reales solo mostramos la imagen; métricas opcionales
    setVitRealImageUrl('/api/model/confusion-matrix-vit-real')
  }, [])

  const imageUrl = activeTab === 'cnn' ? cnnImageUrl : activeTab === 'vit' ? vitImageUrl : vitRealImageUrl
  const metrics = activeTab === 'cnn' ? cnnMetrics : activeTab === 'vit' ? vitMetrics : null
  const overall = activeTab === 'cnn' ? cnnOverall : activeTab === 'vit' ? vitOverall : null
  const loading = activeTab === 'cnn' ? cnnLoading : activeTab === 'vit' ? vitLoading : false
